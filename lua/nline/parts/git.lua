...
function git.changes(icons)
  icons = icons or {}
  return function()
    if vim.api.nvim_buf_get_option(0, "bufhidden") ~= "" or vim.api.nvim_buf_get_option(0, "buftype") == "nofile" then
      return ""
    end
    ...
    local ok, job_result = pcall(function(j)
      return j:sync()
    end, job)
    local ok, result = pcall(function()
      return parse_shortstat_output(vim.trim(job_result[1]))
    end)
    if not ok then
      return ""
    end
    ...
    if result.changed then
      table.insert(output, string.format("%s%s", default_icons.git_changed, result.changed))
    end
    if result.deletions then
      table.insert(output, string.format("%s%s", default_icons.git_deletions, result.deletions))
    end
    if result.insertions then
      table.insert(output, string.format("%s%s", default_icons.git_insertions, result.insertions))
    end
